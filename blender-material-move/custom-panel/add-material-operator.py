bl_info = {
    "name": "Add new material (green sphere)",
    "blender": (2, 80, 0),
    "category": "Material",
}
...
class AddMaterialAndSphere(bpy.types.Operator):
    """My material adding test"""      # Use this as a tooltip for menu items and buttons.
    bl_idname = "material.add_green_sphere"    # Unique identifier for buttons and menu items to reference.
    bl_label = "Add a material(green sphere)"         # Display name in the interface.
...
def menu_func(self, context):
    self.layout.operator(AddMaterialAndSphere.bl_idname)

def register():
    bpy.utils.register_class(AddMaterialAndSphere)
    bpy.types.VIEW3D_MT_object.append(menu_func)  # Adds the new operator to an existing menu.

def unregister():
    bpy.utils.unregister_class(AddMaterialAndSphere)
